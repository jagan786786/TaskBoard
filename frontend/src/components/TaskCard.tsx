  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between text-sm text-gray-500">
        <div className="flex items-center space-x-3">
          {task.assigneeEmail && (
            <div className="flex items-center">
              ...
              <span className="truncate max-w-24">{task.assigneeEmail.split('@')[0]}</span>
            </div>
          )}
        ...
        {task.badge && (
          <div className={`px-2 py-1 rounded-full text-xs font-medium flex items-center ${badgeColors[task.badge]}`}>
            {React.createElement(badgeIcons[task.badge], { className: 'w-3 h-3 mr-1' })}
            {task.badge}
          </div>
        )}
      </div>

...
import { X, Calendar, User, MessageSquare, Send, Edit2, Save, Trash2 } from 'lucide-react';
...
export const TaskModal: React.FC<TaskModalProps> = ({
  ...
  const loadComments = async () => {
    if (!task) return;
    try {
      const comments = await api.getComments(task.id);
  ...
  const handleSave = async () => {
    ...
    try {
      setIsLoading(true);
      const updatedTask = await api.updateTask(task.id, editTask);
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              {isEditing ? (
                ...
                <span className={`inline-block px-3 py-1 rounded-full text-sm font-medium border ${priorityColors[task.priority]}`}>
                  {task.priority}
                </span>
              )}
            </div>
            ...
            <div>
              ...
              {isEditing ? (
                <select
                  ...
                  onChange={(e) => setEditTask({ ...editTask, assigneeId: e.target.value ? Number(e.target.value) : null })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  ...
                  onChange={(e) => setEditTask({ ...editTask, dueDate: e.target.value || null })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                />
              ) : (
                <div className="flex items-center">
